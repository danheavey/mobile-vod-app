...
// this is the test account please comment out wwhen ready to go live
var s_account="channel4flashmobiledev";

// this is the live account please uncomment out wwhen ready to go live
//var s_account="channel4flashmobile";
...
s.linkInternalFilters="javascript:,channel4.com,www.channel4.com,4od.nicebeta.com,nicebeta.com";
...
// Get cntsrc or intcmp Cookie
var omniExecuteOnce1 = false;
var omniExecuteOnce2 = false; 
function getCheckTrackingCookie(t) {
	var omnCookieName = t;
	var omnCookieStart = document.cookie.indexOf(omnCookieName + "=");
	var error = document.cookie.indexOf(omnCookieName + "=;");   
	if (omnCookieStart!=-1 && error==-1){
		var omnCookieLen = omnCookieStart + omnCookieName.length + 1;
		var omnCookieEnd = document.cookie.indexOf(';', omnCookieLen);
		if (omnCookieEnd == -1){
			omnCookieEnd = document.cookie.length;
		}
		omnCookieStart = unescape(document.cookie.substring(omnCookieLen, omnCookieEnd));	  
	}

	return omnCookieStart;
}

//Campaigns
if(!s.campaign  && !omniExecuteOnce1) {
   omnCookieStart = getCheckTrackingCookie("omn_cntsrc"); 
   if (omnCookieStart!=-1){
		s.campaign = s.getValOnce(omnCookieStart,'s_ec_one',0);  
   } else {
   	s.campaign = s.getValOnce(s.getQueryParam('cntsrc'),'s_ec_one',0);
   }
  s.prop10 = s.getAndPersistValue(s.campaign, 's_ecid', 7);
  s.eVar12 = s.campaign;
  s.eVar19 = s.campaign;
  document.cookie = "omn_cntsrc=;path=/;expires=Thu, 01-Jan-1970 00:00:01 GMT";
  document.cookie = "omn_cntsrc=;domain=.channel4.com;path=/;expires=Thu, 01-Jan-1970 00:00:01 GMT";
  omniExecuteOnce1 = true;	
}

// Unified Sources
if (omnCookieStart!=-1) {
    s.eVar14 = omnCookieStart;
} else {
    s.eVar14 = s.getQueryParam('cntsrc');
}
s.eVar21 = s.eVar14;

// Internal Campaigns
if(!s.eVar11 && !omniExecuteOnce2) {
	omnCookieStart = getCheckTrackingCookie("omn_intcmp");
	if (omnCookieStart!=-1){
		s.eVar11 = s.getValOnce(omnCookieStart,'s_ic_one',0);
	} else {
		s.eVar11 = s.getValOnce(s.getQueryParam('intcmp'),'s_ic_one',0);
	}
	s.prop11 = s.getAndPersistValue(s.eVar11, 's_icid', 0);
	document.cookie = "omn_intcmp=;path=/;expires=Thu, 01-Jan-1970 00:00:01 GMT";
	document.cookie = "omn_intcmp=;domain=.4music.com;path=/;expires=Thu, 01-Jan-1970 00:00:01 GMT";
	omniExecuteOnce2 = true;
}

// Previous Page Name
if(!s.prop12) {
  s.prop12 = s.getPreviousValue(s.pageName,'s_c4_gppn','');
}
...
// Get Title Tag
s.prop27 = document.title==""?"Data Not Available":document.title;


		
// Lowercase and Copy to eVars
if(s.prop2){s.prop2 = s.prop2.toLowerCase();}
if(s.prop3){s.prop3 = s.prop3.toLowerCase();}
if(s.prop4){s.prop4 = s.prop4.toLowerCase();}
if(s.prop5){s.prop8 = s.prop8.toLowerCase();}
...
// Page Influence Event
s.events=(s.events)?s.events+",event7":"event7";
/* Visit Number */
	s.prop14 = "visit number_" + s.getVisitNum();
	var scvisitnumber = s.getVisitNum();
    if(s.prop14) 
	{
	    var tempVisitNumber = s.getValOnce(s.prop14,'s.prop14',0);
	    if(scvisitnumber==1) 
		{
	    	s.prop57 ="first visit".toLowerCase();
	    	}
	    if(scvisitnumber>1)
		{
	    	s.prop57 ="repeat".toLowerCase();
	    	}
	    if(tempVisitNumber) 
		{
	    		s.eVar59=tempVisitNumber;
			// Visit Event
			s.events = (s.events)?s.events+",event21":"event21";	
	        	if(scvisitnumber==1) 
			{
	        		s.eVar57 ="first visit".toLowerCase();
	        	}
	        	if(scvisitnumber>1) 
			{
	        		s.eVar57 ="repeat".toLowerCase();
	        	}
	    	}
		}
// Events in sprop
...
+"vari,x=0,a= newArray;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x"
...
/****************************** MODULES *****************************/
/* Module: Media */
s.m_Media_c="='s_media_'+m._in+'_~=new Function(~m.ae(mn,l,\"'+p+'\",~;`H~o.'+f~o.Get~=function(~){var m=this~}^9 p');p=tcf(o)~setTimeouts(~x,x!=2?p:-1,o)}~=parseInt(~m.s.d.getElementsByTagName~ersion"
